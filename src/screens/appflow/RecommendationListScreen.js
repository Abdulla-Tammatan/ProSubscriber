...
import RecommendationListView from "../../views/appflow/RecommendationListView";
...
const RecommendationListScreen = () => {
    ...
    const fetchSuscriptionData = async (email) => {
      ...
      try {
        const response = await axios.request({
          method: "GET", 
          url: `http://192.168.120.71:8080/recommendations/${email}`
        });
        console.log("inside fetch data: ", response.data);
  ...
  if (!loading) {
    return <RecommendationListView recommendations={data} />;
  }
