...
const SubscriptionDetailView = (prop) => {
    ...
    const fetchSuggestCancellation = async (email) => {
      ...
      try {
        const response = await axios.request({
          method: "GET", 
          url: `http://192.168.120.71:8080/usage/suggestCancellation/${prop.subscription.serviceName}/${email}`
        });
  ...
  const deleteSubscription = async () => {
    try {
      const response = await axios.request({
        method: "DELETE", 
        url: `http://192.168.120.71:8080/subscription/${email}/${prop.subscription.serviceName}`
      });
      console.log(response.data);
    }
    catch (error) {
